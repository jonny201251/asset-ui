...
import OperateButton2 from './OperateButton2'
...
export {
  LoadingButton,
  ToolBarButton, OperateButton, OperateButton2,
  ToolBarButtonProcess, OperateButtonProcess,
  BaseList, BaseProTable, BaseProTable2, BaseProTableProcess,
}

//文本框+按钮
const InputButton = (props) => {
  if (props.position === 'ArrayTable') {
    const index = ArrayTable.useIndex()
    const row = ArrayTable.useRecord()
    if (props.form.pattern === 'editable') {
      return <div style={{ display: 'inline-flex', width: '100%' }}>
        <Input style={{ ...props.style }} disabled/>
        <Button onClick={(e) => {
          if (props.onClick) {
