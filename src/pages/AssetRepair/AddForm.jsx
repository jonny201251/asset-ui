...
const InputButton = (props) => {
  ...
  props.form.query('list.' + index + '.*(!repairType,planType,fileList,money,remark,name)').forEach(field => field.setPattern('disabled'))
...
export default (props) => {
  ...
  useEffect(async () => {
    ...
    if (type === 'add') {
      ...
      form.setValues({
        checkName: new Date().Format('yyyyMMdd') + '的维保检申请',
  ...
  const onClick = (index, row) => {
    if (row?.repairType && row?.planType) {
      ...
      form.query('list.' + index + '.*(repairType,planType)').forEach(field => field.validate())
    }
  }
  ...
  return <Form form={form} labelWidth={100} className={styles.placeholder}>
    <SchemaField>
      <SchemaField.Void x-component="FormGrid" x-component-props={{ maxColumns: 4, strictAutoFit: true }}>
        <SchemaField.String name="checkName" required title="维保单名称" x-decorator="FormItem" x-component="Input"/>
      ...
      >
        <SchemaField.Object>
          ...
          <SchemaField.Void x-component="ArrayTable.Column"
                            x-component-props={{ width: 100, title: '操作类别', align: 'center' }}>
            <SchemaField.String
              name="repairType" required x-decorator="FormItem" x-component="Select"
              enum={[
                { label: '维修', value: '维修' },
                { label: '保养', value: '保养' },
                { label: '检测', value: '检测' },
          ...
          <SchemaField.Void x-component="ArrayTable.Column" x-component-props={{ title: '预算金额', align: 'center' }}>
            <SchemaField.Number name="money" required x-decorator="FormItem" x-component="NumberPicker"/>
          </SchemaField.Void>
          <SchemaField.Void x-component="ArrayTable.Column" x-component-props={{ title: '维保检说明', align: 'center' }}>
