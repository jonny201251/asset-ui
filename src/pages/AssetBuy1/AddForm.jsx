...
const InputButton = (props) => {
  ...
  if (!row.planType || row.planType === '计划外') {
    ...
    props.form.query('list.' + index + '.*(!planType,remark,moneyFrom)').forEach(field => {
    ...
    props.form.query('list.' + index + '.*(!planType,remark,name,moneyFrom)').forEach(field => field.setPattern('disabled'))
    return <div style={{ display: 'inline-flex', width: '100%' }}>
...
export default (props) => {
  ...
  useEffect(async () => {
    ...
    if (type === 'add') {
      ...
      form.setValues({
        checkName: new Date().Format('yyyy年MM月dd日') + '的办公营具购置',
  ...
  const onClick = (index, row) => {
    if (row?.planType && row?.planType === '计划内') {
